...
backupsuccess()
{
    ...
    /bin/nvram set ecrs_backup_usb_text="[SUCCESS] - Last run: $TS"
...
then
    ...
    then
        ...
        # Try to do a backup
        if /bin/sh "$BACKUPDIR/nvram-save.sh" -m -i "$BACKUPDIR/nvram-ecrs.ini"
        then
            # Verify archive directory exists
            if [ -d "$ARCHIVEDIR" ]
                # Compress and archive backup
                if /bin/tar -zcf "$BACKUPDIR/archive/$TS.tar.gz" "$BACKUPDIR/backup/*"
                then
